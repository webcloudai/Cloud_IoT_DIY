...
def delete_users_dashboard_with_id(
        dashboard_id:str,
        *,
        userid:str,
    ...
    if not isinstance(userid, str) or len(userid)==0 or not isinstance(dashboards_ds, ObjectsDatasource) or\
       not isinstance(dashboard_id, str) or len(dashboard_id)==0:
        _top_logger.error(f"delete_users_dashboard_with_id: wrong parameters provided to save the dashboard")
        return False

    try:
        return dashboards_ds.remove_object(dashboard_id)
    except Exception as e:
        _top_logger.error(f"delete_users_dashboard_with_id: FAIL to collect dashboards for {userid} with exception {e}")
        return False
...
def lambda_handler(event:dict, context):
    ...
    result = {
        "statusCode": 200,
        "body": json.dumps({
            "save_result": delete_users_dashboard_with_id(
                dashboard_id=dashboard_id, userid=user_id, dashboards_ds=dashboards_ds_s3, **event)
        }),
